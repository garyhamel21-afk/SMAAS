/* ============================================================
   ...
   - news/dashboard: persistent iframes (state preserved)
   ...
   (function () {
    ...
    /* ---------- external view: open in new tab + placeholder panel ---------- */
    ...
    function show(id, fromHash) {
      ...
      // 외부(새 탭) 모듈: 탭만 열고, 허브 화면도 안내 패널로 전환
      if (VIEWS[id].external && !fromHash) {
        openStationTab();
      }

      if (id === current) {
        // 같은 탭을 다시 눌러도 외부 모듈이면 탭 재포커스
